...
Write-Host "`n=== T3: graph-aware RAG (prompt now carries graph) ===" -ForegroundColor Cyan
try {
...
Get-ChildItem -Path . -Recurse -Include _diag_*.py,_fix_*.py -File -ErrorAction SilentlyContinue |
  Where-Object { $_.FullName -notmatch '_trash_|node_modules|\.git\\' } |
  ForEach-Object { Remove-Item $_.FullName -Force }
& ".\push_updates.ps1" "feat: ai_enhancer v2 - graph hint guaranteed in prompt (end-to-end wire proof)"
Write-Host "`n=== BLOCK AU DONE ===" -ForegroundColor Green
