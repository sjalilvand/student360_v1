...
try {
  ...
  Write-Host ("  conversations=" + $q.total_conversations + " | helpful=" + $q.helpful + " | not_helpful=" + $q.not_helpful + " | satisfaction=" + $q.satisfaction_pct + "% | low_conf=" + $q.low_confidence + " | ai_events=" + $q.ai_events)
...
Write-Host "`n=== T4: LLM analyze ===" -ForegroundColor Cyan
try {
  $ab = '{"days":30}'
...
Write-Host "`n=== BLOCK AE DONE ===" -ForegroundColor Green
