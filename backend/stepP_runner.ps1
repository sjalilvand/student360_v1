...
Write-Host "`n=== T1: risk/me (expect factors + reasons) ===" -ForegroundColor Cyan
(Invoke-RestMethod "$base/api/risk/me" -Headers $h -TimeoutSec 15) | ConvertTo-Json -Compress -Depth 4

Write-Host "`n=== T2: risk/list (staff view) ===" -ForegroundColor Cyan
(Invoke-RestMethod "$base/api/risk/list?limit=10" -TimeoutSec 20) | ConvertTo-Json -Compress -Depth 4
...
Write-Host "`n=== BLOCK P DONE ===" -ForegroundColor Green
