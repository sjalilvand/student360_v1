...
Write-Host "`n=== T3: mart thread in logs? ===" -ForegroundColor Cyan
Start-Sleep -Seconds 18
 $thread = Select-String -Path $out -Pattern "mart auto-refresh thread started" -Quiet
if ($thread) { Write-Host "  mart auto-refresh thread STARTED (refresh ~15s after boot)" -ForegroundColor Green }
else { Write-Host "  thread marker not found yet - check log:" -ForegroundColor Yellow; Get-Content $out -Tail 5 }

Write-Host "`n=== T4: peak hour in Tehran time ===" -ForegroundColor Cyan
try {
  $ins = Invoke-RestMethod "$base/api/behavior/insights?days=30" -Headers @{ "X-Student-Number" = "402101001" } -TimeoutSec 20
  Write-Host ("  peak_hour(Tehran)=" + $ins.peak_hour + " | streak=" + $ins.streak_days + " | trend=" + $ins.trend)
} catch { Write-Host ("  FAIL: " + $_.Exception.Message) -ForegroundColor Red }

Write-Host "`n=== T5: graph-aware RAG (word matching - expect codes/chain) ===" -ForegroundColor Cyan
try {
  ...
  Write-Host ("  answer[:500]: " + $ans.Substring(0, [Math]::Min(500, $ans.Length)))
} catch { Write-Host ("  FAIL: " + $_.Exception.Message) -ForegroundColor Red }

Write-Host "`n=== T6: cleanup underscore scripts + PUSH ===" -ForegroundColor Cyan
Set-Location "D:\Projects\university-scheduler"
 $trackedUs = git ls-files | Select-String -Pattern "backend/_" 
if ($trackedUs) { $trackedUs | ForEach-Object { $_.Line } | ForEach-Object { git rm --cached $_ 2>$null | Out-Null }; Write-Host "  untracked helper scripts" }
if (-not (Select-String -Path ".gitignore" -Pattern "backend/_" -Quiet)) {
  Add-Content -Path ".gitignore" -Value "", "backend/_*.py" -Encoding UTF8
}
& ".\push_updates.ps1" "polish: word-based graph matching, Tehran TZ analytics, mart auto-refresh thread"
Write-Host "`n=== BLOCK AR DONE ===" -ForegroundColor Green
