...
Write-Host "`n=== T3: neighbors endpoint ===" -ForegroundColor Cyan
try {
  $n = Invoke-RestMethod ("$base/api/graph/neighbors?code=1044&depth=2") -TimeoutSec 30
  Write-Host ("  center=" + $n.center + " (" + $n.center_title + ") | nodes=" + $n.nodes.Count + " | edges=" + $n.edges.Count)
  $n.nodes | Select-Object -First 5 | ForEach-Object { Write-Host ("    node: " + $_.id + " | " + $_.label + " | type=" + $_.type) }
...
foreach ($f in @("StaffPages.jsx","GraphView.jsx","../../components/GraphView.jsx")) {
  $url = "http://127.0.0.1:5173/src/pages/student360/$f"
  if ($f -like "*components*") { $url = "http://127.0.0.1:5173/src/components/GraphView.jsx" }
  try { $r = Invoke-WebRequest $url -TimeoutSec 10 -UseBasicParsing; Write-Host ("  $f -> OK") -ForegroundColor Green }
...
Get-ChildItem -Path . -Recurse -Include *.bakviz,*.bak3,*.bak2,*.bak -File -ErrorAction SilentlyContinue |
...
# add bak patterns to gitignore
 $gi = ".gitignore"
if (-not (Select-String -Path $gi -Pattern '\*\.bak' -Quiet)) {
  Add-Content -Path $gi -Value "*.bak", "*.bak2", "*.bak3", "*.bakfix", "*.bakviz" -Encoding UTF8
}
git add -A
 $bad = git diff --cached --name-only --diff-filter=ACMR | Where-Object { $_ -match "^\.env$|\.db$" }
if ($bad) { Write-Host ("  ABORT sensitive: " + ($bad -join ", ")) -ForegroundColor Red }
else {
  git commit -m "feat: live graph visualization (react-flow) + neighbors subgraph endpoint" 2>&1 | Select-Object -Last 2
  git push origin main 2>&1 | ForEach-Object { Write-Host $_ }
  if ($LASTEXITCODE -eq 0) { Write-Host "  *** GITHUB UPDATED ***" -ForegroundColor Green }
}

Write-Host "`n=== GAM 1 DONE ===" -ForegroundColor Green
