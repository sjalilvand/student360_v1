# app/api/routes_vote_polls.py  (v4 - complete clean rewrite)
...
def active_polls(survey_type: str = "request", term: str = "1405-1",
    ...
    """)
    rows = db.execute(sql, {"st": survey_type, "t": term, "sid": sid or -1}).mappings().all()
...
def poll_update(body: PollUpdate, db: Session = Depends(get_db)):
    ...
    if body.is_active:
        exists = db.execute(text(
            ...
            "AND term = :t"),
            {"c": body.course_id, "st": body.survey_type, "t": body.term}).first()
        if not exists:
            db.execute(text(
    ...
    db.execute(text(
        "DELETE FROM vote_polls WHERE course_id = :c AND survey_type = :st "
        "AND term = :t"),
