...
if ([string]::IsNullOrWhiteSpace($key)) { Write-Host "  NO KEY!" -ForegroundColor Red }
elseif ($key -eq $old) { Write-Host "  STILL OLD EXPOSED KEY" -ForegroundColor Red }
...
Write-Host "`n=== T3: twin GOOD term (Ali, 12u real courses) ===" -ForegroundColor Cyan
try {
  ...
  Write-Host ("  warnings: " + $r.warnings.Count)
...
Write-Host "`n=== T4: twin BAD term (12u avg 9) ===" -ForegroundColor Cyan
try {
...
foreach ($f in @("Student360Portal.jsx","TwinPage.jsx")) {
...
Write-Host "`n=== BLOCK AG DONE ===" -ForegroundColor Green
