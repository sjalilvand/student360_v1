...
### افزودنی — حذف و حریم خصوصی در نظرسنجی دروس
- GET /api/votes/mine (رأی‌های خود دانشجو با عنوان درس)
- DELETE /api/votes/{id} و DELETE /api/proposals/{id} با بررسی مالکیت (403 برای غیرخودی)
- هاب دانشجو v2.4: کارت «رأی‌های من» + دکمه حذف در رأی‌ها و پیشنهادها + رفرش زنده
- dedup: رأی تکراری همان نوع و پیشنهاد تکراری همان درس/ترم → 409 فارسی
- انتخابگر دروس: منبع واحد offered_courses با GROUP BY (حذف تکراری‌ها)

### افزودنی — مهاجرت پورتال استاد (از Flask زیر بار) + پنل کنترل دسترسی
- مهاجرت داده: ۳۱۱ حساب استاد (auth werkzeug) + ۲۹۷ پیشنهاد غنی‌شده + max_units
- بک‌اند: /api/professor/login (werkzeug-compatible با hashlib خالص) + me/stats + change-password
- CRUD پیشنهادها و دسترس‌بازی (فقط مال خودی، رفع تعارض Student duplicate)
- فرانت: پورتال استاد با UI حرفه‌ای (KPI کارت‌های گرادیانی، گرید هفتگی دسترس‌بازی کلیک‌شو)
- پنل کنترل دسترسی: ماتریس نقش × منو با toggle و reset + منوهای پویای پورتال (sys_permissions)
- ورود ۴کاناله: OTP / SSO / کارشناس / استاد
